...
export class SimulationService {
  ...
  calculateSimulation(sim: Simulation) {
    let resultat = 0;
    const revenuMensuel = sim.salaire + sim.autreRevenu;
    const revenuAnnuel = revenuMensuel * 12;
    let revenu: number;
    const EMI = sim.montant / sim.nbreEcheance;
    const today = new Date(Date.now());
    const age = today.getFullYear() - sim.dateNaissance.getFullYear();
    if (sim.unite === 'Mois') {
    ...
    const pourcentage = (EMI / revenu) * 100;
